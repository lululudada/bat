...
def get_start_number():
    ...
    while True:
        try:
            start_num = int(input("请输入起始编号（例如输入1生成JYJ00001）：").strip())
            if start_num < 1:
                raise ValueError
            return start_num
        except ValueError:
...
def process_images():
        ...
        for filename in files:
            ...
            if filename.lower().endswith(supported_extensions):
                try:
                    ...
                    # 生成新文件名
                    new_filename = f"JYJ{current_number:05d}.jpg"
                    current_number += 1
    ...
    if processed_count > 0:
        ...
        print(f"起始编号: JYJ{start_number:05d}")
        print(f"结束编号: JYJ{(current_number-1):05d}")
